...
export const getDepGraph = (data: DepGraph) => {
  ...
  defs
    ...
    .attr("fill", (d) => d === 0 ? "#FDC659" : d3.interpolateCool(d / maxDistance));
  ...
  const nodeCircle = node
    ...
    .attr("stroke", (d) => d.id === data.nodes[0]?.id
      ? d3.interpolateCool(1 / maxDistance)
      : data.links.every((x) => x.source !== d.id)
        ? "rgba(214, 96, 77)"
        : null)
    .attr("stroke-width", (d) => d.id === data.nodes[0]?.id || data.links.every((x) => x.source !== d.id)
      ? 2
      : null)
    .attr("r", 6)
    .on("mouseover", (e, d) => {
      ...
      const excludedSourceNode = nodeCircle.filter((u) => !targets.includes(u.id) && u.id !== d.id);
      const excludedSourceLabel = nodeLabel.filter((u) => !targets.includes(u.id) && u.id !== d.id);
  ...
  const nodeLabel = node
    ...
    .attr("pointer-events", (d) => d.id === data.nodes[0]?.id ? "none" : "auto")
  ...
  const handleTick = () => {
    ...
    const box = data.nodes.reduce((acc, x) => {
      return [
        Math.min(acc[0], x.x),
        Math.min(acc[1], x.y),
        Math.max(acc[2], x.x),
    ...
    svg.attr("viewBox", [ box[0] - MARGIN.left, box[1] - MARGIN.top, w + MARGIN.right, h + MARGIN.bottom ].join(" "));
